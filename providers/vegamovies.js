...
async function extractSingleVc(vcUrl, referer, targetSeason, targetEp, label, fallbackQ, mediaInfo) {
  ...
  $('a.btn, a').each((_, el) => {
    ...
    if (!href || href === '#' || !href.startsWith('https://') || href.toLowerCase().includes('.fit') || skipBtn(lt)) return;
  ...
  if (!bridgeUrl) {
    const redir = $('a[href*="vcloud.fit"]').filter((_, el) => { const h = $(el).attr('href') || ''; return !h.includes('/api/') && h !== newUrl && h.startsWith('https://'); }).first().attr('href');
