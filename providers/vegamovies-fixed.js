...
const fixUrl = (url, base = BASE_URL) => {
    if (!url) return '';
    const value = String(url).trim();
    if (/^https?:\/\//i.test(value)) return value.replace(/^http:\/\//i, 'https://');
    if (value.startsWith('//')) return 'https:' + value;
    try { return new URL(value, base).toString(); }
    catch (e) { return base.replace(/\/$/, '') + '/' + value.replace(/^\//, ''); }
};
const normalizeQ = q => {
    if (!q)
        return null;
...
function fetchSafe(url_1) {
    return __awaiter(this, arguments, void 0, function* (url, opts = {}, timeout = 20000) {
        ...
        try {
            ...
            return yield fetch(url, Object.assign({}, opts, { headers, signal: controller.signal }));
        }
        catch (e) {
            return null;
        }
...
function makeStream(_, title, url, quality, headers, mediaInfo, fallbackQ = 'HD') {
    ...
    // 1080p limit is applied after sorting all results.
...
function extractNexdriveLinks(html) {
    ...
    $('a[href*="nexdrive"], a[href*="genxfm"], a[href*="fastdl"], a[href*="vcloud"], a[href*="hubcloud"]').each((_, el) => {
        ...
        const href = rawHref ? fixUrl(rawHref, BASE_URL) : '';
...
function extractSingleVc(vcUrl, referer, targetSeason, targetEp, label, fallbackQ, mediaInfo) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        $('a.btn, a').each((_, el) => {
            ...
            href = href && href !== '#' ? fixUrl(href, newUrl) : '';
            const text = ($(el).text() || '').trim(), lt = text.toLowerCase();
            if (!href || !href.startsWith('https://') || href.toLowerCase().includes('.zip') || skipBtn(lt))
                return;
        ...
        $b('a.btn, a').each((_, el) => {
            let href = $b(el).attr('href') || '';
            href = href && href !== '#' ? fixUrl(href, bridgeUrl) : '';
            const text = ($b(el).text() || '').trim(), lt = text.toLowerCase();
            if (!href || !href.startsWith('https://') || href.toLowerCase().includes('.zip') || skipBtn(lt))
                return;
            if (lt.includes('fslv2'))
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const sorted = dedupe(streams)
                .filter(isHubVc)
                .sort((a, b) => (b._resWeight - a._resWeight) || (b._sizeWeight - a._sizeWeight));
            const limited = [];
            let hd1080 = 0;
            for (const stream of sorted) {
                const is1080 = /\b1080P\b/i.test(stream.title || stream.name || '');
                if (is1080) {
                    if (hd1080 >= MAX_1080P) continue;
                    hd1080++;
                }
                limited.push(stream);
            }
