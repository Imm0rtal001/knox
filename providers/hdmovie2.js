...
var BASE = 'https://newhdmovie2.best' // Updated Domain 🌐
var CDN = 'https://hdm.im'
  ...
  return httpGet(url, { 'Referer': BASE + '/' })
    .then(function(html) {
      ...
      while ((articleMatch = articleRegex.exec(html)) !== null) {
        ...
        var linkMatch = articleHtml.match(/href="(https:\/\/newhdmovie2\.best\/movies\/([^"\/]+)\/)"/)
...
function getHdm2Stream(playerUrl) {
  return httpGet(playerUrl, { 'Referer': BASE + '/' })
    .then(function(html) {
      ...
      console.log('[HDMovie2] hdm stream found!')
      return {
        url: finalUrl,
...
function tryGetStream(postId, movieUrl) {
  ...
  function tryNume() {
    ...
    return httpPost(
      ...
      var hdm2Match = cleaned.match(/src="(https:\/\/hdm\.im\/play\?v=[^"]+)"/)
      if (hdm2Match) {
